...
void Prec::construct_level(uint level, const SkylineMatrix& A) {
    ...
    // it changes spectrum on the finest level but we don't use that estimate,
    // though number of PCG iterations increases 16->19
#if 0
    if (level == 0) {
	uint removed = 0;
	uint xs = 0;
	...
	for (uint i = 0; i < nx; i++) 
		...
		for (uint j = k&1; j < ny-1; j += 2) {
		    ...
		    // if (i0 == 0 || i0 == 2) xs++;
		    if ((++(rmap.begin()))->second == 0 || (++(rmap.begin()))->second == 2) xs++;
		    else ys++;
		    // continue;
