...
C-----------------------------------------------------------------------
C
C     CHANGE AGAINST VERSION 1.5, APRIL 2002 (BY KONSTANTIN N. LIPNIKOV)
C
C 1.  V-CYCLE HAS BEEN SYMMETRIZED. THE RELAXATION PROCEDURE RELX HAS BEEN 
C     REPLACED BY TWO PROCEDURES RELXD (OLD RELX) AND RELXU (WITH SYMETRIC
C     ORDER OF COMPUTATIONS). HOWEVER, "FF-RELAXATION (=4)" STILL NOT GIVES 
C     A SYMMETRIC CYCLE.
C
C@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
C
...
C
C
C     SCALAR INPUT PARAMETERS OF AMG1R6:
C
C     THE INPUT PARAMETERS OF AMG1R6 IN THE LIST BELOW ARE ARRANGED
...
C     AMG1R6. CHANGING THEM DOESN'T REQUIRE UNDERSTANDING THE AMG -
...
C     THE AMG1R6 - SUPPLIED STANDARD VALUES SHOULD GIVE SATISFACTORY
C     RESULTS.
C
C         --------------------------------------------------------------
C
...
C                  IF AMG1R6 IS CALLED THE FIRST TIME, ISWTCH HAS TO
...
C
C                    >0: FATAL ERROR (ABNORMAL TERMINATION OF AMG1R6)
C                    <0: NON-FATAL ERROR (EXECUTION OF AMG1R6 CONTINUES)
C
...
C     WORK SPACE:
C
C     THE INTEGER VECTOR IG HAS TO BE PASSED TO AMG1R6 AS WORK SPACE.
C
C-----------------------------------------------------------------------
C
...
C     STANDARD VALUE IS USED BY AMG1R6.
C
C-----------------------------------------------------------------------
C
...
C        (SUBPARAMETER OF NSOLCO). THEN NDRV ISN'T CALLED BY AMG1R6.
...
C        YALE UNITVERSITY.
C
C     4. IN AMG1R6 THERE IS THE PARAMETER LRATIO, DENOTING THE RATIO
C        OF SPACE OCCUPIED BY A DOUBLE PRECISION REAL VARIABLE AND
C        THAT OF AN INTEGER. FOR THE IBM-VERSION LRATIO HAS BEEN SET
...
70    WRITE (IUM,9930)
      IERR = 18
      RETURN
9910  FORMAT (' *** ERROR IN AMG1R6: NDIG TOO SMALL ***')
9920  FORMAT (' *** ERROR IN AMG1R6: ILLEGAL PARAMETER ISWTCH ***')
9930  FORMAT (' *** ERROR IN AMG1R6: ILLEGAL PARAMETER LEVELX ***')
      END
C@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
C
C     GENERAL AMG1R6 SETUP-SUBROUTINES
C
C@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
C.......................................................................
C
...
C
     ...
     +                 MAXGR,LRATIO)
C
C     PREPARATION PHASE OF AMG1R6 (GENERAL PART)
C
...
C@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
C
C     AMG1R6 SETUP ROUTINES (VERSION 3)
C
C@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
C.......................................................................
C
...
C     PHASE OF AMG1R6. ALSO, ICG(I) (IMIN(K)<=I<=IMAX(K)) ARE SET TO
...
C
     ...
     +        IMINW(*),IMAXW(*),IFG(*),ICG(*),NSTCOL(*),NG(*)
      EXTERNAL COARSE,CTIME,INTA,PUTZ,RELXD,RELXU,RESID,VSCALE
      SAVE
C
...
150   DO 170 N=1,NRDX
        DO 160 NL=1,NRDLEN
          CALL RELXD(K,NRDTYP(NL),A,U,F,IA,JA,IW,IMIN,IMAX,IMINW,ICG,
     +              NSTCOL,TIME)
160     CONTINUE
170   CONTINUE
...
200   CALL VSCALE(K,IVSTAR,A,U,F,IA,JA,IW,IMIN,IMAX,IMINW,TIME)
      ...
      DO 215 N=1,NRUX
        DO 210 NL=NRULEN,1,-1
          CALL RELXU(K,NRUTYP(NL),A,U,F,IA,JA,IW,IMIN,IMAX,IMINW,ICG,
     +              NSTCOL,TIME)
210     CONTINUE
215   CONTINUE
...
C
     ...
     +        NSTCOL(*),ESP,PATH,FLAG
      EXTERNAL CTIME,NDRV,RELXD,RELXU,RESID
C
...
400   IF (NRCX.NE.0) THEN
        DO 180 ITER=1,NRCX
          CALL RELXD(M,2,A,U,F,IA,JA,IW,IMIN,IMAX,IMINW,ICG,NSTCOL,TIME)
180     CONTINUE
      ELSE
C
...
181     CONTINUE
        ...
        DO 187 I=1,10
          DO 185 J=1,10
            CALL RELXD(M,2,A,U,F,IA,JA,IW,IMIN,IMAX,IMINW,ICG,NSTCOL,
     +                TIME)
185       CONTINUE
          CALL CTIME(TOLD)
...
C.......................................................................
C
C     RELX (DOWN)                                             SUBROUTINE
C
C.......................................................................
C
      SUBROUTINE RELXD(K,IREL,A,U,F,IA,JA,IW,IMIN,IMAX,IMINW,ICG,NSTCOL,
     +                TIME)
C
...
C.......................................................................
C
C     RELX (UP)                                               SUBROUTINE
C
C.......................................................................
C
      SUBROUTINE RELXU(K,IREL,A,U,F,IA,JA,IW,IMIN,IMAX,IMINW,ICG,NSTCOL,
     +                TIME)
C
C     PERFORMS ONE (PARTIAL) GAUSS-SEIDEL SWEEP ON GRIK K:
C
...
C===> F-RELAXATION
C
100   DO 120 I=IMAX(K),IMIN(K),-1
        IF (ICG(I).GT.0) GOTO 120
        S = F(I)
        DO 110 J=IA(I+1)-1,IA(I)+1,-1
          S = S-A(J)*U(JA(J))
110     CONTINUE
        U(I) = S/A(IA(I))
120   CONTINUE
...
C===> FULL GS RELAXATION
C
200   DO 220 I=IMAX(K),IMIN(K),-1
        S = F(I)
        DO 210 J=IA(I+1)-1,IA(I)+1,-1
          S = S-A(J)*U(JA(J))
210     CONTINUE
        U(I) = S/A(IA(I))
220   CONTINUE
...
C===> C-RELAXATION
C
300   DO 320 I=IMAX(K),IMIN(K),-1
        IF (ICG(I).LE.0) GOTO 320
        S = F(I)
        DO 310 J=IA(I+1)-1,IA(I)+1,-1
          S = S-A(J)*U(JA(J))
310     CONTINUE
        U(I) = S/A(IA(I))
320   CONTINUE
...
C===> FF-RELAXATION
C
400   DO 420 I=IMAX(K),IMIN(K),-1
        IF (ICG(I).NE.0) GOTO 420
        S = F(I)
        DO 410 J=IA(I+1)-1,IA(I)+1,-1
          S = S-A(J)*U(JA(J))
410     CONTINUE
        U(I) = S/A(IA(I))
420   CONTINUE
...
C===> C-RELAXATION
C
      DO 440 I=IMAX(K),IMIN(K),-1
        IF (ICG(I).LE.0) GOTO 440
        S = F(I)
        DO 430 J=IA(I+1)-1,IA(I)+1,-1
          S = S-A(J)*U(JA(J))
430     CONTINUE
        U(I) = S/A(IA(I))
440   CONTINUE
